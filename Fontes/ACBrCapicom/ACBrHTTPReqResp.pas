    ...
    try
      ...
      try
        Header := 'Host: ' + AHost + sLineBreak +
                  'Content-Type: ' + FMimeType + SLineBreak +
                  'Accept-Charset: ' + FCharsets + SLineBreak +
                  'SOAPAction: "' + FSOAPAction + '"' + SLineBreak + SLineBreak;
        ...
        Ok := False;
        Resp.Size := 0;
        if HttpSendRequest(pRequest, nil, 0, Pointer(UTF8Encode(FData)), Length(UTF8Encode(FData))) then
        begin
...
//          end;
        ...
        begin
          ...
          raise EACBrHTTPReqResp.Create('Erro: Requisição não enviada.'+sLineBreak+IntToStr(ErrorCode)+' - '+GetWinInetError(ErrorCode));
        end;
      finally
