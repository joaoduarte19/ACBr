...
type
  { TNFAgXmlReader }
  TNFAgXmlReader = class(TACBrXmlReader)
  private
    FNFAg: TNFAg;
...
//    procedure Ler_gSaldoCred2(const ANode: TACBrXmlNode; gTipoSaldo: TgTipoSaldoCollectionItem);
    ...
    procedure Ler_InfNFAgSupl(const ANode: TACBrXmlNode);
...
procedure TNFAgXmlReader.Ler_Acessante(const ANode: TACBrXmlNode);
var
  sAux: string;
begin
  if not Assigned(ANode) then Exit;
  {
  NFAg.acessante.idAcesso := ObterConteudo(ANode.Childrens.Find('idAcesso'), tcStr);
  NFAg.acessante.idCodCliente := ObterConteudo(ANode.Childrens.Find('idCodCliente'), tcStr);
  NFAg.acessante.tpAcesso := StrTotpAcesso(ObterConteudo(ANode.Childrens.Find('tpAcesso'), tcStr));
  NFAg.acessante.xNomeUC := ObterConteudo(ANode.Childrens.Find('xNomeUC'), tcStr);

  sAux := ObterConteudo(ANode.Childrens.Find('tpClasse'), tcStr);
  NFAg.acessante.tpClasse := tcComercial;
  if sAux <> '' then
    NFAg.acessante.tpClasse := StrTotpClasse(sAux);

  sAux := ObterConteudo(ANode.Childrens.Find('tpSubClasse'), tcStr);
  NFAg.acessante.tpSubClasse := tscResidencial;
  if sAux <> '' then
    NFAg.acessante.tpSubClasse := StrTotpSubClasse(sAux);

  NFAg.acessante.tpFase := StrTotpFase(ObterConteudo(ANode.Childrens.Find('tpFase'), tcStr));
  NFAg.acessante.tpGrpTensao := StrTotpGrpTensao(ObterConteudo(ANode.Childrens.Find('tpGrpTensao'), tcStr));
  NFAg.acessante.tpModTar := StrTotpModTar(ObterConteudo(ANode.Childrens.Find('tpModTar'), tcStr));
  NFAg.acessante.latGPS := ObterConteudo(ANode.Childrens.Find('latGPS'), tcStr);
  NFAg.acessante.longGPS := ObterConteudo(ANode.Childrens.Find('longGPS'), tcStr);
  NFAg.acessante.codRoteiroLeitura := ObterConteudo(ANode.Childrens.Find('codRoteiroLeitura'), tcStr);
  }
end;
...
procedure TNFAgXmlReader.Ler_gGrContrat(const ANode: TACBrXmlNode);
//var
//  Item: TgGrContratCollectionItem;
begin
  if not Assigned(ANode) then Exit;
  {
  Item := NFAg.gGrContrat.New;
...
procedure TNFAgXmlReader.Ler_gJudic(const ANode: TACBrXmlNode);
begin
  if not Assigned(ANode) then Exit;

//  NFAg.gJudic.chNFAg := ObterConteudo(ANode.Childrens.Find('chNFAg'), tcStr);
end;

procedure TNFAgXmlReader.Ler_gMed(const ANode: TACBrXmlNode);
var
...
procedure TNFAgXmlReader.Ler_gNF(const ANode: TACBrXmlNode);
var
  xData: string;
begin
  if not Assigned(ANode) then Exit;
  {
  NFAg.gSub.CNPJ := ObterConteudo(ANode.Childrens.Find('CNPJ'), tcStr);
  NFAg.gSub.serie := ObterConteudo(ANode.Childrens.Find('serie'), tcStr);
  NFAg.gSub.nNF := ObterConteudo(ANode.Childrens.Find('nNF'), tcInt);

  xData := ObterConteudo(ANode.Childrens.Find('CompetEmis'), tcStr);
  if xData <> '' then
  begin
    xData := '01/' + Copy(xData, 5, 2) + '/' + Copy(xData, 1, 4);
    NFAg.gSub.CompetEmis := StrToDate(xData);
  end
  else
    NFAg.gSub.CompetEmis := 0;

  xData := ObterConteudo(ANode.Childrens.Find('CompetApur'), tcStr);
  if xData <> '' then
  begin
    xData := '01/' + Copy(xData, 5, 2) + '/' + Copy(xData, 1, 4);
    NFAg.gSub.CompetApur := StrToDate(xData);
  end
  else
    NFAg.gSub.CompetApur := 0;

  NFAg.gSub.hash115 := ObterConteudo(ANode.Childrens.Find('hash115'), tcStr);
  }
end;

procedure TNFAgXmlReader.Ler_gConsumidor(const ANode: TACBrXmlNode);
var
  i: Integer;
...
begin
  if not Assigned(ANode) then Exit;
  {
  Item := NFAg.gSCEE.gConsumidor.New;
...
procedure TNFAgXmlReader.Ler_gSaldoCred(const ANode: TACBrXmlNode);
var
...
begin
  if not Assigned(ANode) then Exit;
   {
  Item := NFAg.gSCEE.gSaldoCred.New;
...
{
procedure TNFAgXmlReader.Ler_gSaldoCred2(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_gTipoSaldo(const ANode: TACBrXmlNode);
var
  i: Integer;
...
begin
  if not Assigned(ANode) then Exit;
  {
  Item := NFAg.gSCEE.gTipoSaldo.New;
...
procedure TNFAgXmlReader.Ler_gSCEE(const ANode: TACBrXmlNode);
var
  ANodes: TACBrXmlNodeArray;
  i: Integer;
begin
...
//  NFAg.gSCEE.tpPartComp := StrTotpPartComp(ObterConteudo(ANode.Childrens.Find('tpPartComp'), tcStr));
...
procedure TNFAgXmlReader.Ler_gSub(const ANode: TACBrXmlNode);
begin
  if not Assigned(ANode) then Exit;

  NFAg.gSub.chNFAg := ObterConteudo(ANode.Childrens.Find('chNFAg'), tcStr);
  NFAg.gSub.motSub := StrToMotSub(ObterConteudo(ANode.Childrens.Find('motSub'), tcStr));
...
begin
  ...
  // Reforma Tritutária
//  Ler_gCompraGov(NFAg.Ide.gCompraGov, ANode.Childrens.Find('gCompraGov'));
end;
...
procedure TNFAgXmlReader.Ler_NFDet(const ANode: TACBrXmlNode);
var
...
begin
{
  ...
  begin
    ...
    begin
      ...
      ANodeNivel3 := ADetNodes[i].Childrens.Find('gAjusteNFAgAnt');

      if NodeNaoEncontrado(ANodeNivel3) then
        gAjusteNFAgAnt.tpAjuste := taNenhum
      else
      begin
        gAjusteNFAgAnt.tpAjuste := StrTotpAjuste(ObterConteudo(ANodeNivel3.Childrens.Find('tpAjuste'), tcStr));
        gAjusteNFAgAnt.motAjuste := StrToMotAjuste(ObterConteudo(ANodeNivel3.Childrens.Find('motAjuste'), tcStr));
      end;
...
{
procedure TNFAgXmlReader.Ler_Total(const ANode: TACBrXmlNode);
var
  AuxNode: TACBrXmlNode;
begin
  if not Assigned(ANode) then Exit;

  NFAg.Total.vProd := ObterConteudo(ANode.Childrens.Find('vProd'), tcDe2);

  AuxNode := ANode.Childrens.Find('ICMSTot');

  if AuxNode <> nil then
  begin
    NFAg.Total.vBC := ObterConteudo(AuxNode.Childrens.Find('vBC'), tcDe2);
    NFAg.Total.vICMS := ObterConteudo(AuxNode.Childrens.Find('vICMS'), tcDe2);
    NFAg.Total.vICMSDeson := ObterConteudo(AuxNode.Childrens.Find('vICMSDeson'), tcDe2);
    NFAg.Total.vFCP := ObterConteudo(AuxNode.Childrens.Find('vFCP'), tcDe2);
    NFAg.Total.vBCST := ObterConteudo(AuxNode.Childrens.Find('vBCST'), tcDe2);
    NFAg.Total.vST := ObterConteudo(AuxNode.Childrens.Find('vST'), tcDe2);
    NFAg.Total.vFCPST := ObterConteudo(AuxNode.Childrens.Find('vFCPST'), tcDe2);
  end;
  ...
  if AuxNode <> nil then
  begin
    NFAg.Total.vRetPIS := ObterConteudo(AuxNode.Childrens.Find('vRetPIS'), tcDe2);
    NFAg.Total.vRetCOFINS := ObterConteudo(AuxNode.Childrens.Find('vRetCOFINS'), tcDe2);
    NFAg.Total.vRetCSLL := ObterConteudo(AuxNode.Childrens.Find('vRetCSLL'), tcDe2);
    NFAg.Total.vIRRF := ObterConteudo(AuxNode.Childrens.Find('vIRRF'), tcDe2);
  end;

  NFAg.Total.vCOFINS := ObterConteudo(ANode.Childrens.Find('vCOFINS'), tcDe2);
  NFAg.Total.vCOFINSEfet := ObterConteudo(ANode.Childrens.Find('vCOFINSEfet'), tcDe2);
  NFAg.Total.vPIS := ObterConteudo(ANode.Childrens.Find('vPIS'), tcDe2);
  NFAg.Total.vPISEfet := ObterConteudo(ANode.Childrens.Find('vPISEfet'), tcDe2);
  NFAg.Total.vNF := ObterConteudo(ANode.Childrens.Find('vNF'), tcDe2);
  NFAg.Total.vTotDFe := ObterConteudo(ANode.Childrens.Find('vTotDFe'), tcDe2);

  // Reforma Tributária
  Ler_IBSCBSTot(ANode.Childrens.Find('IBSCBSTot'), NFAg.total.IBSCBSTot);
end;

procedure TNFAgXmlReader.Ler_gFat(const ANode: TACBrXmlNode);
var
  xData: string;
  AuxNode: TACBrXmlNode;
begin
  ...
  begin
    xData := '01/' + Copy(xData, 5, 2) + '/' + Copy(xData, 1, 4);
    NFAg.gFat.CompetFat := StrToDate(xData);
  end
  else
    NFAg.gFat.CompetFat := 0;

  NFAg.gFat.dVencFat := ObterConteudo(ANode.Childrens.Find('dVencFat'), tcDat);
  NFAg.gFat.dApresFat := ObterConteudo(ANode.Childrens.Find('dApresFat'), tcDat);
  NFAg.gFat.dProxLeitura := ObterConteudo(ANode.Childrens.Find('dProxLeitura'), tcDat);
  NFAg.gFat.nFat := ObterConteudo(ANode.Childrens.Find('nFat'), tcStr);
  NFAg.gFat.codBarras := ObterConteudo(ANode.Childrens.Find('codBarras'), tcStr);
  NFAg.gFat.codDebAuto := ObterConteudo(ANode.Childrens.Find('codDebAuto'), tcStr);
  NFAg.gFat.codBanco := ObterConteudo(ANode.Childrens.Find('codBanco'), tcStr);
  NFAg.gFat.codAgencia := ObterConteudo(ANode.Childrens.Find('codAgencia'), tcStr);

  AuxNode := ANode.Childrens.Find('enderCorresp');

  if AuxNode <> nil then
  begin
    NFAg.gFat.enderCorresp.xLgr := ObterConteudo(AuxNode.Childrens.Find('xLgr'), tcStr);
    NFAg.gFat.enderCorresp.nro := ObterConteudo(AuxNode.Childrens.Find('nro'), tcStr);
    NFAg.gFat.enderCorresp.xCpl := ObterConteudo(AuxNode.Childrens.Find('xCpl'), tcStr);
    NFAg.gFat.enderCorresp.xBairro := ObterConteudo(AuxNode.Childrens.Find('xBairro'), tcStr);
    NFAg.gFat.enderCorresp.cMun := ObterConteudo(AuxNode.Childrens.Find('cMun'), tcInt);
    NFAg.gFat.enderCorresp.xMun := ObterConteudo(AuxNode.Childrens.Find('xMun'), tcStr);
    NFAg.gFat.enderCorresp.CEP := ObterConteudo(AuxNode.Childrens.Find('CEP'), tcInt);
    NFAg.gFat.enderCorresp.UF := ObterConteudo(AuxNode.Childrens.Find('UF'), tcStr);
    NFAg.gFat.enderCorresp.fone := ObterConteudo(AuxNode.Childrens.Find('fone'), tcStr);
    NFAg.gFat.enderCorresp.email := ObterConteudo(AuxNode.Childrens.Find('email'), tcStr);
  end;
  ...
  if AuxNode <> nil then
  begin
    NFAg.gFat.gPIX.urlQRCodePIX := ObterConteudo(AuxNode.Childrens.Find('urlQRCodePIX'), tcStr);
  end;
end;

procedure TNFAgXmlReader.Ler_gANEEL(const ANode: TACBrXmlNode);
var
  i: Integer;
  ANodes: TACBrXmlNodeArray;
begin
...
procedure TNFAgXmlReader.Ler_gHistFat(const ANode: TACBrXmlNode);
var
...
begin
  ...
  for i := 0 to Length(ANodes) - 1 do
  begin
    Ler_gGrandFat(ANodes[i], NFAg.gANEEL.gHistFat.Count -1);
  end;
end;

procedure TNFAgXmlReader.Ler_gGrandFat(const ANode: TACBrXmlNode; I: Integer);
var
...
begin
  ...
  Item := NFAg.gANEEL.gHistFat[I].gGrandFat.New;
...
begin
  ...
  NFAg.InfAdic.infAdFisco := ObterConteudo(ANode.Childrens.Find('infAdFisco'), tcStr);
  NFAg.InfAdic.infCpl := ObterConteudo(ANode.Childrens.Find('infCpl'), tcStr);
end;

procedure TNFAgXmlReader.Ler_gRespTec(const ANode: TACBrXmlNode);
begin
  if not Assigned(ANode) then Exit;

  NFAg.infRespTec.CNPJ     := ObterConteudo(ANode.Childrens.Find('CNPJ'), tcStr);
  NFAg.infRespTec.xContato := ObterConteudo(ANode.Childrens.Find('xContato'), tcStr);
  NFAg.infRespTec.email    := ObterConteudo(ANode.Childrens.Find('email'), tcStr);
  NFAg.infRespTec.fone     := ObterConteudo(ANode.Childrens.Find('fone'), tcStr);
  NFAg.infRespTec.idCSRT   := ObterConteudo(ANode.Childrens.Find('idCSRT'), tcInt);
  NFAg.infRespTec.hashCSRT := ObterConteudo(ANode.Childrens.Find('hashCSRT'), tcStr);
end;
...
begin
  ...
  sQrCode := ObterConteudo(ANode.Childrens.Find('qrCodNFAg'), tcStr);
  sQrCode := StringReplace(sQrCode, '<![CDATA[', '', []);
  sQrCode := StringReplace(sQrCode, ']]>', '', []);

  NFAg.infNFAgSupl.qrCodNFAg := sQrCode;
end;
...
begin
  ...
  NFAg.procNFAg.tpAmb    := StrToTipoAmbiente(ok, ObterConteudo(ANode.Childrens.Find('tpAmb'), tcStr));
  NFAg.procNFAg.verAplic := ObterConteudo(ANode.Childrens.Find('verAplic'), tcStr);
  NFAg.procNFAg.chDFe    := ObterConteudo(ANode.Childrens.Find('chNFAg'), tcStr);
  NFAg.procNFAg.dhRecbto := ObterConteudo(ANode.Childrens.Find('dhRecbto'), tcDatHor);
  NFAg.procNFAg.nProt    := ObterConteudo(ANode.Childrens.Find('nProt'), tcStr);
  NFAg.procNFAg.digVal   := ObterConteudo(ANode.Childrens.Find('digVal'), tcStr);
  NFAg.procNFAg.cStat    := ObterConteudo(ANode.Childrens.Find('cStat'), tcInt);
  NFAg.procNFAg.xMotivo  := ObterConteudo(ANode.Childrens.Find('xMotivo'), tcStr);
  NFAg.procNFAg.cMsg     := ObterConteudo(ANode.Childrens.Find('cMsg'), tcInt);
  NFAg.procNFAg.xMsg     := ObterConteudo(ANode.Childrens.Find('xMsg'), tcStr);
end;
}
...
begin
  ...
  begin
//    Ler_ProtNFAg(Document.Root.Childrens.Find('protNFAg').Childrens.Find('infProt'));
...
//    Ler_InfNFAgSupl(NFAgNode.Childrens.Find('infNFAgSupl'));
...
{
function TNFAgXmlReader.NodeNaoEncontrado(const ANode: TACBrXmlNode): Boolean;
begin
  Result := not Assigned(ANode);
end;

// Reforma Tributária
procedure TNFAgXmlReader.Ler_gCompraGov(gCompraGov: TgCompraGovReduzido;
...
procedure TNFAgXmlReader.Ler_IBSCBS(const ANode: TACBrXmlNode; IBSCBS: TIBSCBS);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_IBSCBS_gIBSCBS(const ANode: TACBrXmlNode; gIBSCBS: TgIBSCBS);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSUF(const ANode: TACBrXmlNode; gIBSUF: TgIBSUFValores);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSUF_gDif(const ANode: TACBrXmlNode; gDif: TgDif);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSUF_gDevTrib(const ANode: TACBrXmlNode; gDevTrib: TgDevTrib);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSUF_gRed(const ANode: TACBrXmlNode; gRed: TgRed);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSMun(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_gIBSMun_gDif(const ANode: TACBrXmlNode; gDif: TgDif);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSMun_gDevTrib(const ANode: TACBrXmlNode; gDevTrib: TgDevTrib);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSMun_gRed(const ANode: TACBrXmlNode; gRed: TgRed);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gCBS(const ANode: TACBrXmlNode; gCBS: TgCBSValores);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gCBS_gDif(const ANode: TACBrXmlNode; gDif: TgDif);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gCBS_gDevTrib(const ANode: TACBrXmlNode; gDevTrib: TgDevTrib);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gCBS_gRed(const ANode: TACBrXmlNode; gRed: TgRed);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_gIBSCBS_gTribRegular(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_gIBSCredPres(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_gCBSCredPres(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_gTribCompraGov(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_IBSCBSTot(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_IBSCBSTot_gIBS(const ANode: TACBrXmlNode;
  gIBS: TgIBS);
begin
  if not Assigned(ANode) then Exit;
...
procedure TNFAgXmlReader.Ler_IBSCBSTot_gIBS_gIBSUFTot(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_IBSCBSTot_gIBS_gIBSMunTot(const ANode: TACBrXmlNode;
...
procedure TNFAgXmlReader.Ler_IBSCBSTot_gCBS(const ANode: TACBrXmlNode;
  gCBS: TgCBS);
begin
  if not Assigned(ANode) then Exit;
