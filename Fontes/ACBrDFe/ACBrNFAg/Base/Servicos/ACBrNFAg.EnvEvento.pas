...
type
  ...
  TEventoNFAg = class(TACBrXmlWriter)
  private
    FidLote: Int64;
...
constructor TEventoNFAg.Create;
begin
  inherited Create;
...
destructor TEventoNFAg.Destroy;
begin
  FEvento.Free;
...
function TEventoNFAg.CreateOptions: TACBrXmlWriterOptions;
begin
...
function TEventoNFAg.GerarXml: Boolean;
var
...
function TEventoNFAg.Gerar_InfEvento(AIdx: Integer): TACBrXmlNode;
var
  sDoc: string;
begin
  ...
  Result.AppendChild(AddNode(tcStr, 'HP12', 'chNFAg', 44, 44, 1,
                                    Evento[AIdx].FInfEvento.chNFAg, DSC_CHAVE));

  if not ValidarChave(Evento[AIdx].InfEvento.chNFAg) then
    wAlerta('HP12', 'chNFAg', '', 'Chave de NF3-e inválida');
...
function TEventoNFAg.Gerar_DetEvento(AIdx: Integer): TACBrXmlNode;
begin
...
function TEventoNFAg.Gerar_Evento_Cancelamento(AIdx: Integer): TACBrXmlNode;
begin
  Result := CreateElement('evCancNFAg');
...
function TEventoNFAg.GetOpcoes: TACBrXmlWriterOptions;
begin
  Result := TACBrXmlWriterOptions(FOpcoes);
end;

procedure TEventoNFAg.SetEvento(const AValue: TInfEventoCollection);
begin
  FEvento.Assign(AValue);
end;

procedure TEventoNFAg.SetOpcoes(const AValue: TACBrXmlWriterOptions);
begin
  FOpcoes := AValue;
end;

function TEventoNFAg.LerXML(const ACaminhoArquivo: string): Boolean;
var
...
function TEventoNFAg.LerXMLFromString(const AXML: string): Boolean;
var
  RetEventoNFAg: TRetEventoNFAg;
begin
  RetEventoNFAg := TRetEventoNFAg.Create;
  try
    RetEventoNFAg.XmlRetorno := AXML;
    Result := RetEventoNFAg.LerXml;

    with FEvento.New do
    begin
      XML := AXML;

      infEvento.ID := RetEventoNFAg.RetInfEvento.id;
      infEvento.cOrgao := RetEventoNFAg.RetInfEvento.cOrgao;
      infEvento.tpAmb := RetEventoNFAg.RetInfEvento.tpAmb;
//      infEvento.CNPJ := RetEventoNFAg.RetInfEvento.CNPJ;
      infEvento.chNFAg := RetEventoNFAg.RetInfEvento.chNFAg;
//      infEvento.dhEvento := RetEventoNFAg.RetInfEvento.dhEvento;
      infEvento.tpEvento := RetEventoNFAg.RetInfEvento.tpEvento;
      infEvento.nSeqEvento := RetEventoNFAg.RetInfEvento.nSeqEvento;

      infEvento.DetEvento.descEvento := RetEventoNFAg.RetInfEvento.xEvento;
      infEvento.DetEvento.nProt := RetEventoNFAg.RetInfEvento.nProt;
      {
      infEvento.DetEvento.xJust := RetEventoNFAg.RetInfEvento.xJust;
      }
      signature.URI := RetEventoNFAg.signature.URI;
      signature.DigestValue := RetEventoNFAg.signature.DigestValue;
      signature.SignatureValue := RetEventoNFAg.signature.SignatureValue;
      signature.X509Certificate := RetEventoNFAg.signature.X509Certificate;

      FRetInfEvento.Id := RetEventoNFAg.RetInfEvento.Id;
      FRetInfEvento.tpAmb := RetEventoNFAg.RetInfEvento.tpAmb;
      FRetInfEvento.verAplic := RetEventoNFAg.RetInfEvento.verAplic;
      FRetInfEvento.cOrgao := RetEventoNFAg.RetInfEvento.cOrgao;
      FRetInfEvento.cStat := RetEventoNFAg.RetInfEvento.cStat;
      FRetInfEvento.xMotivo := RetEventoNFAg.RetInfEvento.xMotivo;
      FRetInfEvento.chNFAg := RetEventoNFAg.RetInfEvento.chNFAg;
      FRetInfEvento.tpEvento := RetEventoNFAg.RetInfEvento.tpEvento;
      FRetInfEvento.xEvento := RetEventoNFAg.RetInfEvento.xEvento;
      FRetInfEvento.nSeqEvento := RetEventoNFAg.RetInfEvento.nSeqEvento;
      FRetInfEvento.cOrgaoAutor := RetEventoNFAg.RetInfEvento.cOrgaoAutor;
      FRetInfEvento.CNPJDest := RetEventoNFAg.RetInfEvento.CNPJDest;
      FRetInfEvento.emailDest := RetEventoNFAg.RetInfEvento.emailDest;
      FRetInfEvento.dhRegEvento := RetEventoNFAg.RetInfEvento.dhRegEvento;
      FRetInfEvento.nProt := RetEventoNFAg.RetInfEvento.nProt;
      FRetInfEvento.XML := RetEventoNFAg.RetInfEvento.XML;
    end;
  finally
    RetEventoNFAg.Free;
  end;
...
function TEventoNFAg.LerFromIni(const AIniString: string): Boolean;
var
  I: Integer;
  sSecao, sFim: string;
  INIRec: TMemIniFile;
...
{$ENDIF}
  ...
  try
    ...
    begin
      ...
      sFim := INIRec.ReadString(sSecao,'chNFAg', 'FIM');
      ...
      begin
        ...
        infEvento.tpEvento := StrToTpEventoNFAg(ok, INIRec.ReadString(sSecao, 'tpEvento', ''));
