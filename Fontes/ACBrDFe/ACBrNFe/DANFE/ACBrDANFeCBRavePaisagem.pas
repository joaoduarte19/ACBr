...
begin
  with DANFeRave, DANFeRave.ACBrNFe.NotasFiscais.Items[DANFeRave.FNFIndex].NFe, DANFeRave.BaseReport do
   begin
     ...
     Bold:=True;
     PrintXY(PosX+31,PosY+17.8,SeSenao(Ide.TpNF=tnEntrada,'0','1'));
      ...
      begin
         ...
         begin
            if EstaVazio(aProtocolo) then
               aProtocolo:=Trim(procNFe.nProt)+' '+SeSenao(procNFe.dhRecbto<>0,DateTimeToStr(procNFe.dhRecbto),'');
            if ((NFeCancelada) or
      ...
      begin
         ...
         begin
            ...
            begin
               ...
               WideFactor:=BarWidth;
               PrintReadable:=False;
               Text:=LimpaNumero(aChaveContigencia);
         ...
         begin
            SetFont(FontNameUsed,SeSenao(Pos('Courier',FontNameUsed)>0,8,11));
            if LimpaNumero(aProtocolo)='' then
            begin
              Bold:=True;
              FontColor:=clRed;
     ...
     begin
        ...
        for i:=1 to 4 do
         begin
           ...
           SetTab(XX+43,pjRight,19+SeSenao(i=4,-1,0),0,0,0);
           XX:=XX+63;
         end;
        GotoXY(XX,YY);
        NewLine;
...
//                                '  RENAVAM: '+RENAVAM+#13+
   ...
   //                                ' QTDADE: '+FormatFloat(qLote)+#13+
                                   ...
                                   SeSenao(vPMC>0,' PMC: '+FormatFloat(vPMC),'');
                    end;
                   QtdeMin:=QtdeMin+1;
                 end;
              end;
          ...
          PrintTab(FormatFloat(Prod.QCom,SeSenao(Mask_qCom='',NotaUtil.PreparaCasasDecimais(CasasDecimais_qCom),Mask_qCom)));
          PrintTab(FormatFloat(Prod.VUnCom,SeSenao(Mask_vUnCom='',NotaUtil.PreparaCasasDecimais(CasasDecimais_vUnCom),Mask_vUnCom)));
  ...
  begin
    ...
    if Trim(FMemoInfCpl.Text)>'' then
     ...
     begin
        ...
        wInfCpl:=wInfCpl+';'+
        'DATA/HORA INÍCIO: '+SeSenao(ide.dhCont = 0,' ',DateTimeToStr(ide.dhCont))+';'+
        'MOTIVO CONTINGÊNCIA: '+SeSenao(EstaVazio(ide.xJust),' ',ide.xJust);
