...
type
  { TNFComXmlReader }
  TNFComXmlReader = class(TDFeRTCXmlReader)
  private
    FNFCom: TNFCom;
...
begin
  ...
  Ler_pgtoVinc(ANode.Childrens.Find('pgtoVinc'), NFCom.pgtoVinc);
...
begin
  ...
  // Reforma Tritutária
  Ler_gCompraGovReduzido(ANode.Childrens.Find('gCompraGov'), NFCom.Ide.gCompraGov);
end;
...
begin
  ...
  NFCom.Emit.ISUFEmit:= ObterConteudo(ANode.Childrens.Find('ISUFEmit'), tcStr);
...
begin
  ...
  Item.Prod.CNPJCobrTerc := ObterConteudo(ANode.Childrens.FindAnyNs('CNPJCobrTerc'), tcStr);

  Ler_gPagAntecipadoProd(ANode.Childrens.FindAnyNs('gPagAntecipado'), Item.Prod.gPagAntecipado);
end;
