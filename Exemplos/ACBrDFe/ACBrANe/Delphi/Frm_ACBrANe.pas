...
begin
  ...
  cbSSLType.Enabled := (ACBrANe1.Configuracoes.Geral.SSLHttpLib in [httpWinHttp,
    httpOpenSSL]);
end;
...
begin
  try
    if cbCryptLib.ItemIndex <> -1 then
      ACBrANe1.Configuracoes.Geral.SSLCryptLib :=
...
begin
  try
    if cbHttpLib.ItemIndex <> -1 then
      ACBrANe1.Configuracoes.Geral.SSLHttpLib :=
...
begin
  ...
  try
    ...
    cbxExibirErroSchema.Checked := Ini.ReadBool('Geral', 'ExibirErroSchema',
      True);
    ...
    cbSeguradora.ItemIndex := cbSeguradora.Items.IndexOf(Ini.ReadString('Geral',
      'Seguradora', 'ATM'));
    ...
    cbxEmissaoPathANe.Checked := Ini.ReadBool('Arquivos', 'EmissaoPathANe',
      false);
    cbxSepararPorCNPJ.Checked := Ini.ReadBool('Arquivos', 'SepararPorCNPJ',
      false);
    cbxSepararPorModelo.Checked := Ini.ReadBool('Arquivos', 'SepararPorModelo',
      false);
...
procedure TfrmACBrANe.ChecarResposta(aMetodo: TMetodo);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        memoLog.Lines.Add('Código Verificação: ' +
          aResumos[i].CodigoVerificacao);
  ...
  begin
    ...
    begin
      ...
      begin
        memoLog.Lines.Add('Numero Averbação: ' +
          aDadosSeguro[i].NumeroAverbacao);
        memoLog.Lines.Add('CNPJ Seguradora : ' +
          aDadosSeguro[i].CNPJSeguradora);
        memoLog.Lines.Add('Nome Seguradora : ' +
          aDadosSeguro[i].NomeSeguradora);
        ...
        memoLog.Lines.Add('Valor Averbado  : ' +
          FloatToStr(aDadosSeguro[i].ValorAverbado));
...
begin
  ...
  memoLog.Lines.Add('Ambiente  : ' +
    TpAmbToStr(ACBrANe1.Configuracoes.WebServices.Ambiente));
...
begin
  ...
  begin
    ...
    if NaoEstaVazio(edtIntervalo.Text) then
      ...
      edtIntervalo.Text :=
        IntToStr(ACBrANe1.Configuracoes.WebServices.IntervaloTentativas);
...
begin
  ...
  try
    ...
    if LfrmSelecionarCertificado.ShowModal = mrOK then
      edtNumSerie.Text := LfrmSelecionarCertificado.StringGrid1.Cells[0,
        LfrmSelecionarCertificado.StringGrid1.Row];
  finally
...
begin
  ...
  OpenDialog1.Filter :=
    'Arquivos ANe (*-CTe.xml)|*-CTe.xml|Arquivos XML (*.xml)|*.xml|Todos os Arquivos (*.*)|*.*';
...
begin
  ...
  OpenDialog1.Filter :=
    'Arquivos ANe (*-ANe.xml)|*-ANe.xml|Arquivos XML (*.xml)|*.xml|Todos os Arquivos (*.*)|*.*';
  OpenDialog1.InitialDir := ACBrANe1.Configuracoes.Arquivos.PathSalvar;
...
begin
  case ACBrANe1.Status of
    ...
    stANeEnviar:
      begin
        if (FfrmStatus = nil) then
          FfrmStatus := TfrmStatus.Create(Application);
        FfrmStatus.lblStatus.Caption := 'Enviando dados do ANe...';
        FfrmStatus.Show;
        FfrmStatus.BringToFront;
      end;
    ...
    stANeConsultar:
      begin
        if (FfrmStatus = nil) then
          FfrmStatus := TfrmStatus.Create(Application);
        FfrmStatus.lblStatus.Caption := 'Consultando dados do ANe...';
        FfrmStatus.Show;
        FfrmStatus.BringToFront;
      end;
    ...
    stANeEmail:
      begin
        if (FfrmStatus = nil) then
          FfrmStatus := TfrmStatus.Create(Application);
        FfrmStatus.lblStatus.Caption := 'Enviando ANe por e-mail...';
        FfrmStatus.Show;
        FfrmStatus.BringToFront;
      end;
  end;
