...
type
  ...
  private
    ...
    // Chave do HKLM onde ficam RootDir/Personalities da IDE. Normalmente e a
    // mesma de FConfigDataLocation, mas quando a IDE e aberta com bds.exe -r
    // <Perfil> a configuracao do usuario (HKCU) muda de lugar e a informacao
    // da instalacao (HKLM) continua onde estava.
    FHKLMDataLocation: string;
    FRootKey: Cardinal;
  ...
  public
    constructor Create(const AConfigDataLocation: string; ARootKey: Cardinal = 0;
      const AHKLMDataLocation: string = ''); virtual;
    ...
    property HKLMDataLocation: string read FHKLMDataLocation;
  ...
  public
    constructor Create(const AConfigDataLocation: string; ARootKey: Cardinal = 0;
      const AHKLMDataLocation: string = ''); override;
    destructor Destroy; override;
...
constructor TJclBorRADToolInstallation.Create(const AConfigDataLocation: string; ARootKey: Cardinal;
  const AHKLMDataLocation: string);
{$IFDEF MSWINDOWS}
var
  HelpPrefix: string;
{$ENDIF MSWINDOWS}
begin
  ...
  if AHKLMDataLocation = '' then
    FHKLMDataLocation := AConfigDataLocation
  else
    FHKLMDataLocation := AHKLMDataLocation;
...
begin
  ...
  // O HKCU segue o perfil; o HKLM continua na chave real da instalacao.
  HKLMKey := HKLMDataLocation;
  if HKLMKey = '' then
    HKLMKey := Key;
  HKLMGlobalKey := StrEnsureSuffix('\', HKLMKey) + GlobalsKeyName;
  ...
  try
    ...
    Globals.AddStrings(GlobalsBuffer);
    RegGetValueNamesAndValues(HKLM, HKLMGlobalKey, GlobalsBuffer);
    Globals.AddStrings(GlobalsBuffer);
    RegGetValueNamesAndValues(HKLM, HKLMKey, GlobalsBuffer);
    Globals.AddStrings(GlobalsBuffer);
  finally
    GlobalsBuffer.Free;
  end;
...
constructor TJclBCBInstallation.Create(const AConfigDataLocation: string; ARootKey: Cardinal;
  const AHKLMDataLocation: string);
begin
  inherited Create(AConfigDataLocation, ARootKey, AHKLMDataLocation);
