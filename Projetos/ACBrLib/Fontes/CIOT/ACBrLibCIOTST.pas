...
function CIOT_ConfigLerValor (const eSessao, eChave: PAnsiChar; sValor: PAnsiChar;
...
function CIOT_Inicializar(const eArqConfig, eChaveCrypt: PAnsiChar): Integer;
 {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
  Result := LIB_Inicializar(pLib, TACBrLibCIOT, eArqConfig, eChaveCrypt);
end;
...
function CIOT_OpenSSLInfo(const sOpenSSLInfo: PAnsiChar; var esTamanho: Integer
  ): Integer;
 {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
...
function CIOT_UltimoRetorno(const sMensagem: PAnsiChar; var esTamanho: Integer
  ): Integer;
 {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
...
function CIOT_ConfigExportar(const sMensagem: PAnsiChar; var esTamanho: Integer
  ): Integer;
 {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
...
function CIOT_ConfigLerValor(const eSessao, eChave: PAnsiChar;
...
function CIOT_ConfigGravarValor(const eSessao, eChave, eValor: PAnsiChar
  ): Integer;
 {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
...
begin
  try
    ...
    Result := TACBrLibCIOT(pLib^.Lib).CarregarXML(eArquivoOuXML);
  except
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).CarregarINI(eArquivoOuINI);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).ObterXml(AIndex, sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).GravarXml(AIndex, eNomeArquivo, ePathArquivo);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).ObterIni(AIndex, sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).GravarIni(AIndex, eNomeArquivo, ePathArquivo);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).LimparLista;
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).GetPath(sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).ObterCertificados(sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(pLib);
    Result := TACBrLibCIOT(pLib^.Lib).Enviar(sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
