...
<head>
    <meta charset="UTF-8">
    <meta name="ACBrBPe" content="width=device-width, initial-scale=1.0">
    <title>
        <?php
        $title = 'ACBrBPe';
        if (isset($_GET['modo']))
            ...
            <div id="configuracoes" class="panelAba selecionado panelEsquerda-container">
                <div class="tabsPanel">
                    ...
                    <div id="arquivos" class="panelAba panelConfiguracoes-container">
                        ...
                        <div class="grid2Col">
                            <label for="EmissaoPathBPe">Salvar Doc. pelo campo Data de Emissão</label>
                            <input type="checkbox" id="EmissaoPathBPe">
                        </div>
                        <div class="grid2Col">
                        ...
                        <br>
                        <div class="form-group">
                            <label for="PathBPe">Pasta Arquivos BPe</label>
                            <input type="text" id="PathBPe">
                        </div>
                        <div class="form-group">
        ...
        <div class="tabsPanel">
            ...
            <div id="envio" class="panelAba selecionado button-container">
                <div class="grid3Col">
                    ...
                    <input type="button" id="ImprimirPDF" value="Imprimir PDF DABPe">
                    ...
                    <input type="button" id="EnviarEmail" value="Enviar BPe Email">
    ...
    <script>
        ...
        $('#salvarConfiguracoes').on('click', function() {
            const infoData = {
                ...
                EmissaoPathBPe: $('#EmissaoPathBPe').prop('checked') ? 1 : 0,
        ...
        $('#EnviarXml').on('click', function() {
            selecionarArquivo(".xml", function(AeArquivoBPe) {
                chamaAjaxEnviar({
                    metodo: "Enviar",
                    tipoArquivo: "xml",
                    AeArquivoBPe: AeArquivoBPe,
                    AImprimir: 0
                });
            });
        });
        ...
        $('#EnviarIni').on('click', function() {
            selecionarArquivo(".ini", function(AeArquivoBPe) {
                chamaAjaxEnviar({
                    metodo: "Enviar",
                    tipoArquivo: "ini",
                    AeArquivoBPe: AeArquivoBPe,
        ...
        $('#ValidarRegrasdeNegocios').on('click', function() {
            selecionarArquivo(".xml", function(AeArquivoXmlBPe) {
                chamaAjaxEnviar({
                    metodo: "ValidarRegrasdeNegocios",
                    AeArquivoXmlBPe: AeArquivoXmlBPe
                });
            });
        });
        ...
        $('#ConsultarComChave').on('click', function() {
            inputBox("Digite a chave da BPe:", function(eChaveOuBPe) {
                chamaAjaxEnviar({
                    metodo: "Consultar",
                    eChaveOuBPe: eChaveOuBPe,
                    AExtrairEventos: 1
                });
            });
        });
        ...
        $('#EnviarEventoINI').on('click', function() {
            selecionarArquivo(".ini", function(AeArquivoBPe) {
                inputBox("Digite o número do Lote:", function(ALote) {
                    chamaAjaxEnviar({
                        metodo: "EnviarEvento",
        ...
        $('#EnviarEmailEvento').on('click', function() {
            selecionarArquivosSequencial([
                { extensao: ".xml", titulo: "Selecione o XML do BPe" },
                { extensao: ".xml", titulo: "Selecione o XML do Evento" }
            ], function(conteudos) {
                var AeArquivoXmlBPe    = conteudos[0];
        ...
        function chamaAjaxEnviar(infoData) {
            ...
            // Ex: http://localhost/BPe/ACBrBPeBase.php?modo=MT
            // Ex: http://localhost/BPe/ACBrBPeBase.php?modo=ST
            var modo = "<?php echo $modo; ?>";
            if (modo == "")
                modo = "MT";

            $.ajax({
                url: modo + '/ACBrBPeServicos' + modo + '.php',
                type: 'POST',
                data: infoData,
        ...
        function selecionarArquivos(quantidade, extensao, retorno) {
            var input = document.createElement("input");
            input.type = "file";
            input.accept = extensao;
            input.multiple = (quantidade !== 1); // permite múltiplos quando quantidade != 1
            ...
            input.onchange = function(event) {
                ...
                if (quantidade && arquivos.length < quantidade) {
                    alert("Selecione " + quantidade + " arquivo(s).");
                    return;
                }

                var conteudos = new Array(arquivos.length);
                var pendentes = arquivos.length;

                arquivos.forEach(function(file, i) {
                    var reader = new FileReader();
                    reader.onload = function(e) {
                        conteudos[i] = e.target.result;
                        pendentes--;
                        if (pendentes === 0) {
                            retorno(conteudos); // array puro, sem nenhuma interpretação de conteúdo
                        }
                    };
        ...
        function processaRetornoConfiguracoes(response) {
            if (response.dados) {
                ...
                $('#EmissaoPathBPe').prop('checked', response.dados.EmissaoPathBPe == 1);
