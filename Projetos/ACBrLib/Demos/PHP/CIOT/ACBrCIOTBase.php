...
<head>
    <meta charset="UTF-8">
    <meta name="ACBrCIOT" content="width=device-width, initial-scale=1.0">
    <title>
        <?php
        $title = 'ACBrCIOT';
        if (isset($_GET['modo']))
            ...
            <div id="configuracoes" class="panelAba selecionado panelEsquerda-container">
                <div class="tabsPanel">
                    ...
                    <div id="geral" class="panelAba selecionado panelConfiguracoes-container">
                        ...
                        <div class="form-group">
                            ...
                            <select id="versaoDF" name="versaoDF">
                                <option value="0" selected>ve500</option>
                            </select>
                        </div>
                        <div class="grid2Col">
                        ...
                        <div class="form-group">
                            <label for="integradoraCIOT">Integradora</label>
                            <select id="integradoraCIOT" name="integradoraCIOT">
                                <option value="0" selected>iNone</option>
                        ...
                        <div class="grid2Col">
                            <div class="form-group">
                                <label for="integradoraUsuario">Usuário</label>
                                <input type="text" id="integradoraUsuario">
                            </div>
                            <div class="form-group">
                                <label for="integradoraSenha">Senha</label>
                                <input type="password" id="integradoraSenha">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="integradoraHash">Hash</label>
                            <input type="password" id="integradoraHash">
                        </div>
                        <div class="form-group">
                            <label for="integradoraToken">Token</label>
                            <input type="password" id="integradoraToken">
                        </div>
                    </div>
                    ...
                    <div id="arquivos" class="panelAba panelConfiguracoes-container">
                        ...
                        <div class="grid2Col">
                            <label for="EmissaoPathCIOT">Salvar Documento pelo campo Dt. Emissão</label>
                            <input type="checkbox" id="EmissaoPathCIOT">
                        </div>
                        <div class="grid2Col">
    ...
    <script>
        ...
        $('#salvarConfiguracoes').on('click', function () {
            const infoData = {
                ...
                integradoraCIOT: $('#integradoraCIOT').val(),
                integradoraUsuario: $('#integradoraUsuario').val(),
                integradoraSenha: $('#integradoraSenha').val(),
                integradoraHash: $('#integradoraHash').val(),
                integradoraToken: $('#integradoraToken').val(),
                SSLType: $('#SSLType').val(),
                timeout: $('#timeout').val(),
                ...
                EmissaoPathCIOT: $('#EmissaoPathCIOT').prop('checked') ? 1 : 0,
        ...
        $('#EnviarIni').on('click', function () {
            selecionarArquivo(".ini", function (AeArquivoCIOT) {
                chamaAjaxEnviar({
                    metodo: "Enviar",
                    tipoArquivo: "ini",
                    AeArquivoCIOT: AeArquivoCIOT
                });
            });
        });
        ...
        function chamaAjaxEnviar(infoData) {
            ...
            // Ex: http://localhost/CIOT/ACBrCIOTBase.php?modo=MT
            // Ex: http://localhost/CIOT/ACBrCIOTBase.php?modo=ST
            var modo = "<?php echo $modo; ?>";
            if (modo == "")
                modo = "MT";

            infoData.integradoraToken = $('#integradoraToken').val();

            $.ajax({
                url: modo + '/ACBrCIOTServicos' + modo + '.php',
                type: 'POST',
                data: infoData,
        ...
        function processaResponseGeral(retorno) {
            if (retorno.mensagem) {
                ...
                $('#integradoraToken').val(retorno.tokenRetornado);
            }
            else
        ...
        function processaRetornoConfiguracoes(response) {
            if (response.dados) {
                ...
                $('#integradoraCIOT').val(response.dados.integradoraCIOT);
                $('#integradoraUsuario').val(response.dados.integradoraUsuario);
                $('#integradoraSenha').val(response.dados.integradoraSenha);
                $('#integradoraHash').val(response.dados.integradoraHash);
                $('#integradoraToken').val(response.dados.integradoraToken);
                ...
                $('#EmissaoPathCIOT').prop('checked', response.dados.EmissaoPathCIOT == 1);
