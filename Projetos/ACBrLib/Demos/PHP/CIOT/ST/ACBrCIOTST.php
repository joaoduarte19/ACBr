...
{
    $retorno = $ffi->CIOT_Inicializar($iniPath, "");
...
{
    $retorno = $ffi->CIOT_ConfigLer($eArqConfig);
...
function ConfigLerValor($ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->CIOT_ConfigLerValor($eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->CIOT_ConfigGravarValor($eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[9048]");
...
{
    $retorno = $ffi->CIOT_ConfigGravar($eArqConfig);
    $sMensagem = FFI::new("char[9048]");
...
function UltimoRetorno($ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        ...
        $resposta = $ffi->CIOT_UltimoRetorno($sMensagem, FFI::addr($esTamanho));
...
function OpenSSLInfo($ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_OpenSSLInfo($sMensagem, FFI::addr($esTamanho));
...
function ObterCertificados($ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_ObterCertificados($sMensagem, FFI::addr($esTamanho));
...
function CarregarINI($ffi, $eArquivoOuINI, &$retornoGeral)
{
    ...
    $retorno = $ffi->CIOT_CarregarINI($eArquivoOuINI);
...
function Enviar($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 144768;
    $sMensagem = FFI::new("char[144768]");
    $retorno = $ffi->CIOT_Enviar($sMensagem, FFI::addr($esTamanho));

    if ($retorno !== 0) {
        if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro ao enviar CIOT", 1) != 0)
            return -10;
    }
...
function CarregarXmlCIOT($ffi, $eArquivoOuXML, &$retornoGeral)
{
    $sMensagem = FFI::new("char[144768]");
    $retorno = $ffi->CIOT_CarregarXML($eArquivoOuXML);

    if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro ao carregar o XML da CIOT", 1) != 0)
        return -10;
...
function ObterXml($ffi, $AIndex, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_ObterXml($AIndex, $sMensagem, FFI::addr($esTamanho));
...
function GravarXml($ffi, $AIndex, $eNomeArquivo, $ePathArquivo, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_GravarXml($AIndex, $eNomeArquivo, $ePathArquivo);

    if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro gravar o xml da CIOT.", 1) != 0)
        return -10;
...
function ObterIni($ffi, $AIndex, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 144768;
    $sMensagem = FFI::new("char[144768]");
    $retorno = $ffi->CIOT_ObterIni($AIndex, $sMensagem, FFI::addr($esTamanho));
...
function GravarIni($ffi, $AIndex, $eNomeArquivo, $ePathArquivo, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_GravarIni($AIndex, $eNomeArquivo, $ePathArquivo);
...
function LimparListaCIOT($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_LimparLista();

    if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro ao limpar a lista de CIOTs.", 1) != 0)
        return -10;
...
function GetPath($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->CIOT_GetPath($sMensagem, FFI::addr($esTamanho));
