...
<head>
    <meta charset="UTF-8">
    <meta name="ACBrDCe" content="width=device-width, initial-scale=1.0">
    <title>
        <?php
        $title = 'ACBrDCe';
        if (isset($_GET['modo']))
...
<body>
        ...
        <div class="tabsPanel">
            <div class="tabAbas">
                <button class="selecionado"
                    onclick="ativaAba(event, 'configuracoes', 'panelEsquerda')">Configurações</button>
            ...
            <div id="configuracoes" class="panelAba selecionado panelEsquerda-container">
                <div class="tabsPanel">
                    <div class="tabAbas">
                        <button class="selecionado"
                            onclick="ativaAba(event, 'geral', 'panelConfiguracoes')">Geral</button>
                    ...
                    <div id="arquivos" class="panelAba panelConfiguracoes-container">
                        ...
                        <div class="grid2Col">
                            <label for="EmissaoPathDCe">Salvar Documento pelo campo Dt. Emissão</label>
                            <input type="checkbox" id="EmissaoPathDCe">
                        </div>
                        <div class="grid2Col">
            ...
            <div id="documento-auxiliar" class="panelAba panelEsquerda-container">
                ...
                <div class="form-group">
                    <label for="PathPDF">Pasta dos PDFs</label>
                    <input type="text" id="PathPDF">
                </div>
                <label>Margens</label>
                <div class="grid4Col">
                    <div>
                        <label for="MargemDireita">Direita</label>
                        <input type="number" id="MargemDireita" value="4">
                    </div>
                    <div>
                        <label for="MargemEsquerda">Esquerda</label>
                        <input type="number" id="MargemEsquerda" value="4">
                    </div>
                    <div>
                        <label for="MargemSuperior">Superior</label>
                        <input type="number" id="MargemSuperior" value="7">
                    </div>
                    <div>
                        <label for="MargemInferior">Inferior</label>
                        <input type="number" id="MargemInferior" value="7">
                    </div>
                </div>
    ...
    <div class="cfgPanelDireita">
        <div class="tabsPanel">
            <div class="tabAbas">
                <button class="selecionado" onclick="ativaAba(event, 'envio', 'panelMetodos')">Envio</button>
                <button onclick="ativaAba(event, 'consultas', 'panelMetodos')">Consultas</button>
                <button onclick="ativaAba(event, 'eventos', 'panelMetodos')">Eventos</button>
            </div>
            <div id="envio" class="panelAba selecionado panelMetodos-container">
                <div class="grid3Col">
                    ...
                    <input type="button" id="ImprimirPDF" value="Imprimir PDF DADCe">
                    ...
                    <input type="button" id="GerarChave" value="Gerar Chave DCe">
                    <input type="button" id="EnviarEmail" value="Enviar DCe Email">
                    <input type="button" id="LimparRepostas" value="Limpar Respostas">
                </div>
            </div>
            <div id="consultas" class="panelAba panelMetodos-container">
                <div class="grid3Col">
                    <input type="button" id="statusServico" value="Status do Serviço">
                    <input type="button" id="ConsultarComChave" value="Consultar com Chave">
                    <input type="button" id="ConsultarComXml" value="Consultar carregando Xml">
                </div>
            </div>
            <div id="eventos" class="panelAba panelMetodos-container">
                <div class="grid2Col">
                    <input type="button" id="CancelarDCe" value="Cancelamento DCe">
    ...
    <script>
        function ativaAba(event, abaId, panel) {
            ...
            if (panel === 'panelMetodos') {
                const abaSelecionada = document.querySelector(`#${abaId}.panelMetodos-container`);
                const outrasAbas = document.querySelectorAll('.panelMetodos-container');
                outrasAbas.forEach(container => container.classList.remove('selecionado'));
        ...
        $('#salvarConfiguracoes').on('click', function () {
            const infoData = {
                ...
                EmissaoPathDCe: $('#EmissaoPathDCe').prop('checked') ? 1 : 0,
        ...
        $('#ConsultarComChave').on('click', function () {
            inputBox("Digite a chave do DCe:", function (eChaveOuDCe) {
                chamaAjaxEnviar({
                    metodo: "Consultar",
                    eChaveOuDCe: eChaveOuDCe,
                    AExtrairEventos: 1
                });
            });
        });

        $('#ConsultarComXml').on('click', function () {
            selecionarArquivo(".xml", function (eChaveOuDCe) {
                chamaAjaxEnviar({
                    metodo: "Consultar",
                    eChaveOuDCe: eChaveOuDCe,
                    AExtrairEventos: 1
        ...
        $('#ImprimirPDF').on('click', function () {
            selecionarArquivo(".xml", function (AeArquivoXmlDCe) {
                chamaAjaxEnviar({
                    metodo: "ImprimirPDF",
                    AeArquivoXmlDCe: AeArquivoXmlDCe
                });
            });
        });
        ...
        $('#ImprimirEventoPDF').on('click', function () {
            selecionarArquivosSequencial([
                { extensao: ".xml", titulo: "Selecione o XML do DCe" },
                { extensao: ".xml", titulo: "Selecione o XML do Evento" }
            ], function(conteudos) {
                var AeArquivoXmlDCe    = conteudos[0];
        ...
        $('#EnviarSincronoXml').on('click', function () {
            selecionarArquivo(".xml", function (AeArquivoDCe) {
                inputBox("Digite o Lote:", function (ALote) {
                    chamaAjaxEnviar({
                        metodo: "Enviar",
                        tipoArquivo: "xml",
                        AeArquivoDCe: AeArquivoDCe,
                        ALote: ALote,
                        AImprimir: 0,
                        AZipado: 0
                    });
        ...
        $('#EnviarSincronoIni').on('click', function () {
            selecionarArquivo(".ini", function (AeArquivoDCe) {
                inputBox("Digite o Lote:", function (ALote) {
                    chamaAjaxEnviar({
                        metodo: "Enviar",
                        tipoArquivo: "ini",
                ...
                inputBox("Digite a justificativa:", function (AeJustificativa) {
                    inputBox("Digite o CNPJ:", function (AeCNPJCPF) {
                        inputBox("Digite o número do Lote:", function (ALote) {
                            inputBox("Digite o Tipo de Emitente:", function (AEmitenteDCe) {
                                chamaAjaxEnviar({
                                    metodo: "CancelarDCe",
                                    AeChave: AeChave,
        ...
        $('#EnviarEmailEvento').on('click', function () {
            selecionarArquivosSequencial([
                ...
                var AeXmlDCe    = conteudos[0];
                var AeXmlEvento = conteudos[1];

                inputBox("Digite o endereço do destinatário:", function (AePara) {
                    inputBox("Enviar PDF? (0)Não (1) Sim", function (AEnviaPDF) {
                        chamaAjaxEnviar({
                            metodo: "EnviarEmailEvento",
                            AeXmlEvento: AeXmlEvento,
                            AeXmlDCe: AeXmlDCe,
                            AePara: AePara,
                            AEnviaPDF: AEnviaPDF,
                            AeAssunto: $('#emailAssunto').val(),
        ...
        function chamaAjaxEnviar(infoData) {
            ...
            // Ex: http://localhost/DCe/ACBrDCeBase.php?modo=MT
            // Ex: http://localhost/DCe/ACBrDCeBase.php?modo=ST
            var modo = "<?php echo $modo; ?>";
            if (modo == "")
                modo = "MT";

            $.ajax({
                url: modo + '/ACBrDCeServicos' + modo + '.php',
                type: 'POST',
                data: infoData,
        ...
        function processaRetornoConfiguracoes(response) {
            if (response.dados) {
                ...
                $('#EmissaoPathDCe').prop('checked', response.dados.EmissaoPathDCe == 1);
                ...
                $('#MargemDireita').val(response.dados.MargemDireita);
                $('#MargemEsquerda').val(response.dados.MargemEsquerda);
                $('#MargemSuperior').val(response.dados.MargemSuperior);
                $('#MargemInferior').val(response.dados.MargemInferior);
