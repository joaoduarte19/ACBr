...
class ConsultarCobrancaImediataPage extends StatefulWidget {
  const ConsultarCobrancaImediataPage({super.key, required this.plugin});

  final ACBrPixCDPlugin plugin;

  @override
  State<ConsultarCobrancaImediataPage> createState() => _ConsultarCobrancaImediataPageState();
}

class _ConsultarCobrancaImediataPageState extends State<ConsultarCobrancaImediataPage> {
  ...
  Future<void> _executar() async {
    ...
    try {
      ...
      final r = await widget.plugin.consultarCobrancaImediata(
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        FilledButton(
          onPressed: _executar,
          child: const Text('Consultar Cobrança Imediata'),
        ),
        const SizedBox(height: 10),
        ResponseBox(
