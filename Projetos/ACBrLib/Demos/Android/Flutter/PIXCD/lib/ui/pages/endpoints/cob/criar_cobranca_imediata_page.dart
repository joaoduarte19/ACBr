...
class CriarCobrancaImediataPage extends StatefulWidget {
  const CriarCobrancaImediataPage({super.key, required this.plugin});

  final ACBrPixCDPlugin plugin;

  @override
  State<CriarCobrancaImediataPage> createState() => _CriarCobrancaImediataPageState();
}

class _CriarCobrancaImediataPageState extends State<CriarCobrancaImediataPage> {
  ...
  Future<void> _executar() async {
    ...
    setState(() {});
    try {
      final r = await widget.plugin.criarCobrancaImediata(
        criarCobrancaImediata: _iniController.text,
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        const Text('Ini para Criar Cobrança Imediata'),
        ...
        FilledButton(
          onPressed: _executar,
          child: const Text('Criar Cobrança Imediata'),
        ),
        const SizedBox(height: 10),
        ResponseBox(
